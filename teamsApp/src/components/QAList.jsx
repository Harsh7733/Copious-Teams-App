...
import { stylesforQAList } from '../../styles/styles';
            ...
            const fetchTasks = async () => {
                ...
                for (const section of sectionData) {
                    try {
                        ...
                        const filteredTasks = taskResponse.data.filter(task => task.status !== 'Completed' && task.isDelete === false && task.sentToQA);
    ...
    return (
        ...
        >
            <View style={{ alignItems: 'center', marginBottom: 5}}>
                <Text style={{ fontSize: 20, fontWeight: 'bold', color: 'white' }}>-: QA Dashboard :-</Text>
            </View>
            {sections.length === 0 ? (
                <Text style={stylesforQAList.noSections}>No sections available.</Text>
            ) : (
                sections.map((section) => (
                    <View key={section.id}>
                        <View style={stylesforQAList.header}>
                            <Text style={stylesforQAList.headerText}>{section.sectionName || 'Unnamed Section'}</Text>
                                ...
                                {tasks[section.id] && tasks[section.id].length > 0 ? (
                                    tasks[section.id].map((task) => (
                                        <TouchableOpacity key={task.id} onPress={() => openTaskDetailsModal(task, section.sectionName)}>
                                            <View style={stylesforQAList.taskContainer}>
                                                <Text style={stylesforQAList.task}>Task: {task.taskName}</Text>
                                    ...
                                    <Text style={stylesforQAList.noTasks}>No active tasks available.</Text>
                                )}
                            </View>
                        </Collapsible>
                    </View>
