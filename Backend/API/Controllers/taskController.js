...
const createTask = async (req, res) => {
    ...
    try {
        ...
        if (tagsIds.length > 0) {
            const validTagIds = tagsIds.map(id => parseInt(id, 10)).filter(id => !isNaN(id));

            if (validTagIds.length === 0) {
                return res.status(400).json({ message: 'No valid tag IDs provided.' });
            }

            const tags = await Tag.findAll({ where: { id: validTagIds } });
            const tagIdsInDb = tags.map(tag => tag.id);
            const invalidTagIds = validTagIds.filter(tagId => !tagIdsInDb.includes(tagId));
